...
public class Membership extends EventSourcingAggregate<MembershipId> {
    ...
    public Membership(MembershipId id, MembershipPeriod period, SubscriptionDefinitionId subscriptionDefinitionId, ContactId subscriberId, Collection<ContactId> additionalSubscriberIds) {

        SubscriptionDefinition definition = period.getSubscriptionDefinitionById(subscriptionDefinitionId)
                .orElseThrow(() -> new IllegalStateException("definition " + subscriptionDefinitionId + " does not exist in period " + period.getId()));

        if ((additionalSubscriberIds.size() + 1) > definition.getMaxSubscribers()) {
            throw new IllegalStateException("a maximum of " + definition.getMaxSubscribers() + " is possible for this subscription type");
        }

        apply(new MembershipCreatedEvent(id, period.getId(), definition.getSubscriptionDefinitionId(), subscriberId, additionalSubscriberIds));
