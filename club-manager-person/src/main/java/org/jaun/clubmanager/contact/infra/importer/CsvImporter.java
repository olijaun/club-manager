...
import org.jaun.clubmanager.contact.application.resource.BasicDataDTO;
import org.jaun.clubmanager.contact.application.resource.ContactDataDTO;
import org.jaun.clubmanager.contact.application.resource.NameDTO;
import org.jaun.clubmanager.contact.application.resource.PersonDTO;
...
public class CsvImporter {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<PersonDTO> contacts = new ArrayList<>(records.size());
        ...
        for (CSVRecord record : records) {
            ...
            if (!StringUtils.isEmpty(bezbisString)) {
                ...
                if (bezbisDate.getYear() >= 2018) {
                    contacts.add(personDTO1);
                    if (personDTO2 != null) {
                        contacts.add(personDTO2);
                    }
                }
            }
        }
        ...
        Map<String, PersonDTO> set = contacts.stream().collect(Collectors.toMap(PersonDTO::getId, Function.identity()));

        for (PersonDTO personDTO : contacts) {
            save(personDTO);
        }
    }
    ...
    private static void save(PersonDTO personDTO) {
        ...
        WebTarget target = client.target("http://localhost:9001/api/contacts").path(personDTO.getId());
        ...
        if (response.getStatusInfo().getFamily().equals(Response.Status.Family.SUCCESSFUL)) {
            ...
            System.out.println("failed with code: " + response.getStatus() + ", contact: " + personDTO);
        }
    }
}
